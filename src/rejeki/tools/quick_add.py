...
def quick_add(db: Database, text: str) -> dict:
    amount = _parse_amount(text)
    if not amount:
        raise ValueError(f"Tidak bisa mendeteksi nominal dari: '{text}'")

    accounts = db.fetchall("SELECT id, name, type FROM accounts ORDER BY name")
    envelopes = db.fetchall("SELECT id, name, icon FROM envelopes WHERE type = 'expense' ORDER BY name")
